...
export default class Board extends Component {
    ...
    floodFill_(grid, row, col, initialColour) {
        grid[row][col].colour = 1; // TODO colour wheel

        for(let m = 1; m <= 7; m+=2) {
            let dr = Math.floor(m/3) - 1, dc = m%3 - 1;
            if(!this.oob(row+dr, col+dc) && grid[row+dr][col+dc].colour === initialColour) this.floodFill_(grid, row+dr, col+dc, initialColour);
        }
    }
    ...
    floodFill(row, col, initialColour) {
        if(initialColour !== 1) {
            let {grid} = this.state;
            this.floodFill_(grid, row, col, initialColour);
